...
void Callback1(const turtlesim::Pose::ConstPtr & position)
{
...
void Callback2(const turtlesim::Pose::ConstPtr & position)
{
	...
	pos1.x=position->x;
	pos1.y=position->y;
	pos1.theta=position->theta;
	pos1.linear_velocity=position->linear_velocity;
	pos1.angular_velocity=position->angular_velocity;
}
int main(int agrc,char **agrv)
{
	system("rosservice call spawn 2.772 5.5444 1.571 turtle2");
	...
	ros::Subscriber sub = n.subscribe("array1",0,Calback);
	while(arrx[0]==-1)
		ros::spinOnce();
	printf("PID 2");
	...
	ros::Publisher pub1 = n.advertise<geometry_msgs::Twist>("turtle2/cmd_vel",1000);
	ros::Publisher pub = n.advertise<geometry_msgs::Twist>("turtle1/cmd_vel",1000);
	
	ros::Subscriber sub1= n.subscribe("turtle1/pose",0,Callback1);
	ros::Subscriber sub2= n.subscribe("turtle2/pose",0,Callback2);
	ros::Rate rate(1);
	ros::Rate rate1(0.1);
	geometry_msgs::Twist cammand,cammand1;
	cammand1.angular.z=-3.142/2;
	cammand1.linear.x=3.142*5.5444/4;
	...
	{
		ros::spinOnce();
		if(sqrt(pow(pos.y-pos1.y,2)+pow(pos.x-pos1.x,2))<50/50.0)
		{
			i=0;
			pub1.publish(cammand1);
			cammand.angular.z=0;
			cammand.linear.x=0;
			pub.publish(cammand);
			arrx[0]=-1;
